...
//! 只增加客户端开销）。hook 脚本把 stdin payload 追加到 `~/.buddy2api/hook-events.jsonl`，
//! 由后端消费（见 `rate_limit_events.rs`）。
//!
//! 三条硬约束（安装会改写用户真实配置，必须守住）：
//! - **幂等**：重复安装不产生重复条目；
//! - **写前备份**：安装前把原文件原样复制到 `~/.buddy2api/hook-backups/`；
...
//! 单测一律注入临时目录，不触碰真实 `~/.codebuddy`、`~/.workbuddy`、`~/.workbuddy-ai`、`~/.buddy2api`。
//!
...
const STORE_DIR_NAME: &str = ".buddy2api";
...
const LEGACY_SH_BODY: &str = "#!/bin/sh\npayload=$(cat)\nprintf '%s\\n' \"$payload\" >> \"$HOME/.buddy2api/hook-events.jsonl\"\nprintf '{}'\n";
...
const LEGACY_CMD_BODY: &str = concat!(
    ...
    "if ($d.Trim().Length -gt 0) { Add-Content -LiteralPath (Join-Path $env:USERPROFILE '.buddy2api\\hook-events.jsonl') ",
...
impl HookLayout {
    ...
    ///
    /// 迁移时只能按这条具体路径识别旧条目，不能用 `.buddy2api` / `hook.cmd`
    /// 等宽泛片段认领第三方命令。
...
mod tests {
    ...
    /// 临时目录下的限额监听配置路径（绝不触碰真实 `~/.buddy2api`）。
    ...
    fn hook_command_quotes_the_script_path() {
        let interpreter = if cfg!(windows) { "bash" } else { "sh" };
        let path = Path::new("/Users/a b/.buddy2api/hook.sh");
        assert_eq!(
            hook_command(path),
            format!("{interpreter} '/Users/a b/.buddy2api/hook.sh'")
        );
        // 单引号路径按 POSIX 写法转义 —— 裸拼会破坏引号配对，事件被静默丢弃
        // （2026-09-20 审查反例）。
        let quoted = Path::new("/Users/o'brien/.buddy2api/hook.sh");
        assert_eq!(
            hook_command(quoted),
            format!("{interpreter} '/Users/o'\\''brien/.buddy2api/hook.sh'")
        );
    }
    ...
    fn hook_command_uses_bash_on_windows() {
        let path = Path::new(r"C:\Users\a b\.buddy2api\hook.sh");
        assert_eq!(hook_command(path), "bash '/c/Users/a b/.buddy2api/hook.sh'");
        let quoted = Path::new(r"C:\Users\o'brien\.buddy2api\hook.sh");
        assert_eq!(
            hook_command(quoted),
            "bash '/c/Users/o'\\''brien/.buddy2api/hook.sh'"
        );
    }
