...
pub const GITHUB_OWNER: &str = "bringCool";
pub const GITHUB_REPO: &str = "buddy2api";
...
pub fn load_github_config() -> Value {
    ...
    // 旧版本配置可能残留上游仓库（changexbc/workbuddy-switch 等）；迁移到本项目实际仓库。
    let legacy = [
        ("changexbc", "workbuddy-switch"),
        ("changexbc", "buddy2api"),
    ];
    if legacy.iter().any(|(o, r)| *o == owner && *r == repo) {
...
async fn fetch_manifest_version(
    ...
    headers.insert("User-Agent".to_string(), "buddy2api".to_string());
...
mod tests {
    ...
    fn updater_manifest_urls_macos_skips_duplicate_fallback() {
        let urls = updater_manifest_urls("bringCool", "buddy2api", "macos", "aarch64");
        assert_eq!(
            urls,
            vec![
                "https://github.com/bringCool/buddy2api/releases/latest/download/latest.json",
                "https://github.com/bringCool/buddy2api/releases/latest/download/latest-macos-aarch64.json",
            ]
        );
    }
    ...
    fn updater_manifest_urls_windows_keeps_macos_compat() {
        let urls = updater_manifest_urls("bringCool", "buddy2api", "windows", "x86_64");
        assert_eq!(
            urls,
            vec![
                "https://github.com/bringCool/buddy2api/releases/latest/download/latest.json",
                "https://github.com/bringCool/buddy2api/releases/latest/download/latest-windows-x86_64.json",
                "https://github.com/bringCool/buddy2api/releases/latest/download/latest-macos-x86_64.json",
            ]
        );
    }
