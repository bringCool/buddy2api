//! hook 信号消费：把 `~/.buddy2api/hook-events.jsonl` 里的 `Stop` / `FinalStop` payload
//! 变成限额台账条目。
//!
...
fn attribute(
    ...
    match source {
        ...
        HookSource::WorkBuddy(variant) => {
            ...
            let accounts: Vec<Value> = ctx.accounts.iter()
                .filter(|acc| account::variant_of(acc) == variant)
                .cloned()
                .collect();
            let ids = account::unique_account_ids_by_uid(&accounts);
            if let Some(account_id) = uid.and_then(|uid| ids.get(&uid).cloned()) {
...
mod tests {
    ...
    #[test]
    fn workbuddy_events_do_not_guess_between_organizations() {
        let fixture = Fixture::new();
        std::fs::write(
            fixture.auth_file(),
        ...
        let accounts = vec![
            json!({"id": "personal", "uid": UID_A, "variant": "cn"}),
            json!({"id": "company", "uid": UID_A, "variant": "cn", "enterpriseId": "company"}),
        ];
        let ctx = fixture.context(accounts, &absent_lookup);
        fixture.append(&[stop_payload(
            "s-shared", "hy3", &fixture.workbuddy_transcript("s-shared"),
        )]);
        let mut state = State::default();
        consume_with(&ctx, &mut state, 1_000);
        assert!(state.events.is_empty());
    }
