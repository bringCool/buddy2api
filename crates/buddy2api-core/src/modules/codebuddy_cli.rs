...
//! 两种模式都复用 buddy2api 的 WorkBuddy 账号库，但保持独立的当前账号。
...
/// buddy2api 的旧 helper，允许直接原地升级，并清理旧文件。
...
mod tests {
  ...
  "env": { "HTTPS_PROXY": "http://127.0.0.1:7890" }
    ...
    fn codebuddy_cli_process_args_match_package_and_skip_ide() {
        ...
        assert!(!is_codebuddy_cli_process_args(
            "node /Users/me/Documents/github-project/buddy2api/src-tauri"
        ));
    }
    ...
    fn helper_selects_active_account_and_fails_without_selected_token() {
        ...
        let output = Command::new("node")
            ...
            .env("BUDDY2API_ACCOUNTS_FILE", &accounts_file)
            .output()
            .unwrap();
