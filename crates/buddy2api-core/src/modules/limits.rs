...
pub(crate) fn account_by_session(
    ...
    let accounts: Vec<Value> = account::load_accounts()
        .into_iter()
        .filter(|acc| account::variant_of(acc) == variant)
        .collect();
    let uid_to_account = account::unique_account_ids_by_uid(&accounts);
...
///
/// CLI 与 IDE 的来源不限定账号档位；多个组织共用 uid 时不靠 uid 猜测归属。
fn account_id_by_uid() -> HashMap<String, String> {
    account::unique_account_ids_by_uid(&account::load_accounts())
}
...
/// [`save_rate_limit_config`] 的可注入路径版本（单测不得触碰真实 `~/.buddy2api`）。
...
mod tests {
    ...
    fn quoted_marker(dir: &Path) -> String {
        format!(
            "'{}'",
            dir.join(".buddy2api").join("hook.sh").to_string_lossy()
        )
    }
