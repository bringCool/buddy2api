...
/// 会话操作涉及的路径集合：工具存储根（`~/.buddy2api`）与档位数据根。
///
/// 生产入口用 [`SessionPaths::for_variant`]；单测注入临时目录，
/// 绝不触碰真实 `~/.buddy2api` 或 WorkBuddy 数据目录。
#[derive(Clone, Debug)]
...
fn account_id_for_uid(paths: &SessionPaths, uid: &str) -> Option<String> {
    ...
    account::unique_account_ids_by_uid(&accounts).remove(uid)
}
...
mod tests {
    //! 会话复制/关联/恢复的端到端单测。
    //!
    //! 所有用例都在临时目录里构造数据根与存储根，绝不读写真实的 `~/.buddy2api`
    //! 或 WorkBuddy 数据目录。
    ...
    fn temp_root(name: &str) -> PathBuf {
        std::env::temp_dir().join(format!(
            "buddy2api_session_root_{}_{name}",
            uuid::Uuid::new_v4().simple()
        ))
    }
    ...
    /// 未登录时拒绝；同 uid 的组织共用会话，跳过且不写任何东西。
    #[test]
    fn copy_rejects_missing_login_and_skips_shared_uid() {
        ...
        .expect("同 uid 的组织应跳过复制");
        assert!(report["skipped"].as_str().unwrap().contains("共用同一个 uid"));
        assert_eq!(report["copied"], json!([]));
