...
//! 负责读写账号库（`~/.buddy2api/accounts.json`）。
...
/// 导出文件名白名单：只允许 `buddy2api-accounts-YYYY-MM-DD.json` 这类由前端生成的文件名。
...
mod tests {
    ...
    fn export_file_name_rejects_traversal_and_non_json() {
        ...
        assert!(validate_export_file_name("buddy2api-accounts-2026-08-21.json").is_ok());
    }
