...
pub(crate) fn is_self_image_name(name: &str) -> bool {
    ...
    stem.eq_ignore_ascii_case("buddy2api")
}

/// 精确匹配本档位的 WorkBuddy 映像，禁止子串命中其它档位或 buddy2api。
...
foreach ($hive in $unHives) {
  Get-ChildItem -LiteralPath $hive | ForEach-Object {
    ...
    if (-not $dn) { return }
    $dnl = [string]$dn
    if ($dnl -match 'buddy2api|buddy2api') { return }
...
// - 排除自身 pid 与 args 含 buddy2api / buddy2api 的 PID（自排除）。
...
/// 过滤：排除自身 pid；排除 args 含 buddy2api / buddy2api 的 PID。
...
pub(crate) fn filter_ps_rows(
    ...
    for line in stdout.lines() {
        ...
        if args.contains("buddy2api") || args.contains("buddy2api") {
            continue;
        }
...
mod tests {
    ...
    fn self_image_names_are_detected() {
        assert!(is_self_image_name("buddy2api"));
        assert!(is_self_image_name("buddy2api.exe"));
        assert!(is_self_image_name("BUDDY2API.EXE"));
        assert!(is_self_image_name(r"C:\apps\buddy2api.exe"));
    ...
    fn workbuddy_image_name_is_exact_not_substring() {
        ...
        assert!(!is_workbuddy_image_name("buddy2api", WbVariant::Cn));
        assert!(!is_workbuddy_image_name(
    ...
    fn variant_image_names_never_cross_match() {
        ...
        for variant in WbVariant::ALL {
            for name in ["buddy2api.exe", "buddy2api", "BUDDY2API.EXE", "Buddy2API.exe"] {
                assert!(
...
1003|buddy2api|C:\\apps\\buddy2api.exe
";
    ...
    fn registry_probe_script_is_variant_scoped() {
        ...
        assert!(ai.contains("buddy2api|buddy2api"), "自排除保留");
    ...
    fn process_rows_drop_self_and_crashpad() {
        let stdout = "\
1001|buddy2api|C:\\apps\\buddy2api.exe
1002|buddy2api|
...
";
    ...
    fn only_switcher_process_is_not_workbuddy_running() {
        let stdout = "4400|buddy2api.exe|C:\\Users\\Zhou\\AppData\\Local\\Programs\\buddy2api\\buddy2api.exe\n";
        let kept =
            filter_windows_workbuddy_rows(&parse_windows_process_rows(stdout), WbVariant::Cn);
        ...
        let csv = "\
\"buddy2api.exe\",\"4400\",\"Console\",\"1\",\"10,000 K\"
\"buddy2api.exe\",\"4401\",\"Console\",\"1\",\"8,000 K\"
";
...
C:\Users\Zhou\AppData\Local\Programs\buddy2api\buddy2api.exe
...
"#;
    ...
    fn macos_ps_filter_matches_case_sensitively_and_excludes_self() {
        ...
        // 5005 args 含 buddy2api：非自身 pid 也按自排除规则剔除。
        let stdout = format!(
            "{self_pid} /Applications/buddy2api.app/Contents/MacOS/buddy2api\n\
             ...
             5005 /opt/tool/buddy2api/helper run\n"
        );
