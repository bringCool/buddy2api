//! buddy2api CLI：npm 安装形态的入口。
//!
//! ```bash
//! buddy2api              # 启动本地服务 + 打开浏览器 webui
//! buddy2api serve        # 只起服务不开浏览器（--port / --no-open）
//! buddy2api status       # 终端输出当前账号
//! buddy2api version      # 版本号
//! ```
...
fn print_status(variant: WbVariant) {
    ...
    println!("buddy2api v{}", update::APP_VERSION);
...
async fn main() {
    ...
    match cmd {
        ...
        "version" | "--version" | "-V" => {
            println!("buddy2api {}", env!("CARGO_PKG_VERSION"));
        }
