...
pub fn router() -> Router {
    Router::new()
        ...
        .merge(buddy2api_core::modules::proxy_http::router())
        .route("/api/proxy/models", get(api_proxy_models))
        .route(
            "/api/proxy/config",
...
async fn api_proxy_config() -> Response {
    ...
    json_ok(json!({
        ...
        "running": openai_proxy::proxy_enabled(),
        "baseUrl": "/v1",
        "config": config,
...
async fn api_save_proxy_config(Json(body): Json<Value>) -> Response {
    match openai_proxy::save_proxy_config(body.get("config").unwrap_or(&body)) {
        Ok(()) => api_proxy_config().await,
        Err(e) => json_err(e, StatusCode::BAD_REQUEST),
    }
}
