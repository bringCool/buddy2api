...
- **当前限制**：国际版会话复制与同步按本机数据目录能力检测开放；Token 统计在国际版数据目录缺失 `projects/` 时显示为空。
...
> 历史遗留：早期版本写入的 `~/.buddy2api/cli_switch_history.jsonl` 不再被读取，也不会被自动删除（不清理用户数据）。
