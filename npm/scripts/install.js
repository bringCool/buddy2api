// buddy2api postinstall：从「平台包」复制本平台二进制。
//
// 平台分包（esbuild 模式）：二进制发布在独立 npm 包（buddy2api-<platform>-<arch>），
// 主包声明为 optionalDependencies，安装时 npm 自动装好平台包，postinstall 只需复制——
// 不依赖 GitHub，国内镜像（npmmirror）也能稳定安装。
//
// 环境变量覆盖：
//   BUDDY2API_BINARY=<本地二进制路径>  本地开发/离线安装（直接复制，不联网）
const fs = require("fs");
...
const PLATFORM_PKG = `buddy2api-${process.platform}-${process.arch}`;

if (!FILE) {
  console.warn(
    `buddy2api: 跳过平台 ${process.platform}-${process.arch}（当前不支持），` +
      `可手动下载二进制后放置到 bin/ 目录`,
  );
  process.exit(0);
}
...
function fail(msg) {
  console.error(`buddy2api install: ${msg}`);
  console.error(
    "安装失败。请确认安装了对应平台包（npm 会自动装），或设置 BUDDY2API_BINARY 指向本地二进制。",
  );
  process.exit(1);
}
...
function copyFrom(src) {
  ...
  console.log(
    `buddy2api: 二进制就绪 → ${target} (${(size / 1048576).toFixed(1)}MB)`,
  );
}

async function main() {
  // 1) 本地二进制覆盖（开发/离线）
  if (process.env.BUDDY2API_BINARY) {
    const local = path.resolve(process.env.BUDDY2API_BINARY);
    if (fs.existsSync(local)) return copyFrom(local);
    return fail(`BUDDY2API_BINARY 指向的文件不存在: ${local}`);
  }

  // 2) 从平台包复制（node_modules/buddy2api-<platform>-<arch>/bin/<file>）
  try {
    ...
    if (fs.existsSync(target)) {
      console.log("buddy2api: 二进制已存在，跳过");
      return;
    }
    return fail(
